...
export const RatingFilter: React.FC<RatingProps> = ({ activeRating, setActiveRating }) => {
    ...
    return (
        <Box>
            ...
            {
                /**
                 * @comment FormGroupStyles is not a descriptive name, avoid such things
                 * also with the * import Styles part is redundant
                 * */
            }
                ...
                {[5, 4, 3, 2, 1].map((rating) => (
                    <FormControlLabel key={rating} control={<Radio value={rating} checked={activeRating === rating} onChange={onChange} />} label={
                        <Stack alignItems='center' flexDirection={'row'} gap={1} >
                            ...
                            <Typography variant='caption' sx={styles.RatingTextStyles}>{+rating - 1}&nbsp;+</Typography>
                        </Stack>
                    } />
                ))}
            </RadioGroup>
